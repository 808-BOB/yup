...
class SupabaseStorage implements IStorage {
  ...
  constructor() {
    ...
    if (!supabaseUrl || !supabaseKey) {
      throw new Error('SUPABASE_PROJECT_URL and SUPABASE_API_KEY must be set');
    }
  ...
  async getUserByLinkedInId(linkedinId: string): Promise<User | undefined> {
    const { data, error } = await this.supabase.from('users').select('*').eq('linkedin_id', linkedinId).single();
    if (error) return undefined;
    return data || undefined;
  }
  ...
  async updateUser(id: string, userData: Partial<User>): Promise<User | undefined> {
    const { data, error } = await this.supabase.from('users').update(userData).eq('id', id).select().single();
    if (error) return undefined;
    return data || undefined;
  }

  async updateUserBranding(id: string, brandData: { brand_theme?: string, logo_url?: string }): Promise<User | undefined> {
    const { data, error } = await this.supabase.from('users').update(brandData).eq('id', id).select().single();
    if (error) return undefined;
    return data || undefined;
  }

  async updateStripeCustomerId(userId: string, customerId: string): Promise<User | undefined> {
    const { data, error } = await this.supabase.from('users').update({ stripe_customer_id: customerId }).eq('id', userId).select().single();
    if (error) return undefined;
    return data || undefined;
  }

  async updateStripeSubscriptionId(userId: string, subscriptionId: string): Promise<User | undefined> {
    const { data, error } = await this.supabase.from('users').update({ stripe_subscription_id: subscriptionId }).eq('id', userId).select().single();
    if (error) return undefined;
    return data || undefined;
  }
  ...
  async updateUserStripeInfo(userId: string, stripeData: { stripe_customer_id?: string, stripe_subscription_id?: string }): Promise<User | undefined> {
    const { data, error } = await this.supabase.from('users').update(stripeData).eq('id', userId).select().single();
    if (error) return undefined;
    return data || undefined;
  }

  async getUserByStripeCustomerId(customerId: string): Promise<User | undefined> {
    const { data, error } = await this.supabase.from('users').select('*').eq('stripe_customer_id', customerId).single();
    if (error) return undefined;
    return data || undefined;
  }
  ...
  async updateUserLinkedIn(id: string, linkedinData: { linkedin_id?: string, linkedin_access_token?: string, linkedin_profile_url?: string, linkedin_connections?: string }): Promise<User | undefined> {
    const { data, error } = await this.supabase.from('users').update(linkedinData).eq('id', id).select().single();
    if (error) return undefined;
    return data || undefined;
  }
  ...
  async updateEvent(id: number, eventUpdate: Partial<InsertEvent>): Promise<Event | undefined> {
    const { data, error } = await this.supabase.from('events').update(eventUpdate).eq('id', id).select().single();
    if (error) return undefined;
    return data || undefined;
  }
  ...
  async getEventBySlug(slug: string): Promise<Event | undefined> {
    const { data, error } = await this.supabase.from('events').select('*').eq('slug', slug).single();
    if (error) return undefined;
    return data || undefined;
  }

  async getUserEvents(userId: string): Promise<Event[]> {
    const { data, error } = await this.supabase.from('events').select('*').eq('host_id', userId);
    if (error) return [];
    return data || [];
  }

  async createInvitation(eventId: number, userId: string): Promise<void> {
    await this.supabase.from('invitations').insert({ event_id: eventId, user_id: userId });
  }

  async getEventInvitations(eventId: number): Promise<string[]> {
    const { data, error } = await this.supabase.from('invitations').select('user_id').eq('event_id', eventId);
    if (error) return [];
  ...
  async createResponse(response: InsertResponse): Promise<Response> {
    const { data, error } = await this.supabase.from('responses').insert(response).select().single();
  ...
  async getResponsesByEvent(eventId: number): Promise<Response[]> {
    const { data, error } = await this.supabase.from('responses').select('*').eq('event_id', eventId);
    if (error) return [];
    return data || [];
  }
  ...
  async getUserEventResponse(eventId: number, userId: string | null): Promise<Response | undefined> {
    ...
    const { data, error } = await this.supabase.from('responses').select('*').eq('event_id', eventId).eq('user_id', userId).single();
    if (error) return undefined;
    return data || undefined;
  }

  async getEventResponses(eventId: number): Promise<{ yupCount: number; nopeCount: number; maybeCount: number }> {
    const { data, error } = await this.supabase.from('responses').select('response_type').eq('event_id', eventId);
  ...
  async getResponseById(id: number): Promise<Response | undefined> {
    const { data, error } = await this.supabase.from('responses').select('*').eq('id', id).single();
    if (error) return undefined;
    return data || undefined;
  }

  async updateResponse(id: number, responseUpdate: Partial<InsertResponse>): Promise<Response | undefined> {
    const { data, error } = await this.supabase.from('responses').update(responseUpdate).eq('id', id).select().single();
    if (error) return undefined;
    return data || undefined;
  }

  async deleteResponse(id: number): Promise<void> {
    await this.supabase.from('responses').delete().eq('id', id);
  }
  ...
  async getEventCount(): Promise<number> {
    const { count, error } = await this.supabase.from('events').select('*', { count: 'exact', head: true });
    if (error) return 0;
    return count || 0;
  }

  async getResponseCount(): Promise<number> {
    const { count, error } = await this.supabase.from('responses').select('*', { count: 'exact', head: true });
    if (error) return 0;
    return count || 0;
  }
}
